...
def parse_book(text: str) -> dict:
    ...
    if rating_tag and rating_tag.get("class") and len(
            rating_tag["class"]) > 1:
...
def get_books_urls(book_catalogue_url: str) -> list:
    ...
    Returns:
        Список строковых URL, каждый из которых ведёт на страницу книги.
    """
...
def get_interval(from_page: int, to_page: int) ->  tuple[int, int]:
    ...
    if to_page is not None and to_page > max_page_number:
...
if __name__ == "__main__":
    scrape_books(from_page=50, to_page=51)
    # scrape_by_schedule()
